...
class SwiftRadioUITests: XCTestCase {
    ...
    let hamburgerMenu = XCUIApplication().navigationBars["Boosh.FM"].buttons["icon hamburger"]
    ...
    func testMainStationsView() {
        ...
        playButton.tap()
        assertPlaying()
        app.navigationBars["Boosh.FM"].buttons["Back"].tap()
        assertStationOnMenu(stationName)
        app.navigationBars["Boosh.FM"].buttons["btn nowPlaying"].tap()
        waitForStationToLoad()
